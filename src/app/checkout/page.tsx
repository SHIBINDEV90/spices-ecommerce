    ...
    try {
        ...
        if (data.paymentMethod === 'razorpay') {
            ...
            if (typeof window === 'undefined' || !(window as any).Razorpay) {
                throw new Error('Payment gateway SDK is loading. Please try again in 3 seconds.');
            }

            if (!data.key || data.key === 'rzp_test_placeholder') {
                alert('Notice: Razorpay keys are not yet configured in .env.local. Order #' + data.orderId + ' was recorded as pending.');
                setLoading(false);
                return;
            }
            ...
            const options = {
                ...
                handler: async function (response: any) {
                    // IMPORTANT: We DO NOT consider the order paid yet!
                    // We send the signature to our server for HMAC SHA256 validation.
                    try {
