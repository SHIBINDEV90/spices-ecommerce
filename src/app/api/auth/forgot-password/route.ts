...
export async function POST(req: Request) {
  try {
    ...
    // Send transactional email via unified email service (SMTP or Resend)
    ...
    if (emailResult.delivered) {
      console.log(`[Forgot Password] Reset email successfully delivered via ${emailResult.provider} to ${user.email}`);
    } else {
      console.warn(`[Forgot Password] Reset email could not be delivered: ${emailResult.error}`);
    }
